...
const CHUNK_INEFFICIENT = "chunk_inefficient";
...
class UserList extends Component {
    ...
    handleFilterValueChange(value) {
        ...
        if (value !== "") {
            dataTable[0].forEach((i, index) => {
                i.map(e => {
                    if (
                        e.id.toString().indexOf(value) >= 0 ||
                        e.name.toLowerCase().indexOf(value.toLowerCase()) >=
                            0 ||
                        e.nickname.toLowerCase().indexOf(value.toLowerCase()) >=
                            0
                    ) {
                        searchData.push(e);
                    }
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if (
            prevProps.LoginReducer.allUsers.data !== LoginReducer.allUsers.data
        ) {
            this.handleUsers();
        } else if (prevProps.LoginReducer.newUser !== LoginReducer.newUser) {
            this.addUser(LoginReducer.newUser.data);
        }
    }
    ...
    handleUsers(index) {
        const { LoginReducer, getUserForId, showModal } = this.props;
        /*renombrar esta constante a algo signicativo */
        const array = LoginReducer.allUsers.data;
        if (array) {
            array.map(i => {
                i.action = [
                    <IconButton
    ...
    render() {
        const { helperReducer, showModal } = this.props;
        return (
            <div style={styles.container}>
                ...
                {helperReducer.showModal && (
                    <ModalFormUser
                        showModal={helperReducer.showModal}
                        userData={this.addUser}
                        returnUser={this.returnUser}
                    />
                )}
                {helperReducer.showAlert && (
                    <Alert
                        showAlert={this.handleDelete}
                        info={"Desea eliminar al usuario?"}
                    />
                )}
            </div>
        ); /*else {
